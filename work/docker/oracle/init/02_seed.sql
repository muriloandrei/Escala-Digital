...
select 1, pagina, 1, 1, 1, 1, 1, 1, 1, sysdate from (
  select 'escalas' pagina from dual union all select 'escalas-funcionarios' from dual union all select 'funcionarios' from dual union all select 'secoes' from dual union all select 'turnos-secao' from dual union all select 'historico' from dual union all select 'regras' from dual union all select 'tipos-descanso' from dual union all select 'horarios-padrao' from dual union all select 'integracao-rm' from dual union all select 'acessos' from dual union all select 'liberacao-secoes' from dual union all select 'roles' from dual union all select 'configuracoes' from dual
);
insert into SGN_ESC_PERFIL_PERMISSAO (PERFIL_ID, PAGINA, PODE_VISUALIZAR, PODE_CRIAR, PODE_EDITAR, PODE_OFICIALIZAR, PODE_REPROCESSAR, PODE_EXCLUIR, PODE_ADMINISTRAR, DT_HR_INCL)
